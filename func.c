...
int battle_pokemon_len(Player *p)
{
    ...
    for (i = 0; i < 6; i++)
    {
        if (p->battle_pokemon[i].attribute != 0)
            num++;
    }
    return num;
}
...
void attack_skill(char skill_name[], int i, Pokemon *x, Pokemon *y)
{
    ...
    if (x->attribute == 1)
    {
        if (y->attribute == 2)
            damage *= 1.25;
        else if (y->attribute == 4 || y->attribute == 5)
            damage *= 0.75;
    }
    else if (x->attribute == 2)
    {
        if (y->attribute == 3)
            damage *= 1.25;
        else if (y->attribute == 1 || y->attribute == 5)
            damage *= 0.75;
    }
    else if (x->attribute == 3)
    {
        if (y->attribute == 4)
            damage *= 1.25;
        else if (y->attribute == 2 || y->attribute == 5)
            damage *= 0.75;
    }
    else if (x->attribute == 4)
    {
        if (y->attribute == 1)
            damage *= 1.25;
        else if (y->attribute == 3 || y->attribute == 5)
            damage *= 0.75;
    }
...
void skill_setting(Pokemon ***p_addr)
{
    ...
    for (i = 0; i < 4; i++)
    {
        for (j = 0; j < 4; j++)
        {
            for (k = 0; k < 3; k++)
            {
                if ((*p_addr)[i][j].skill_info->type == 0)
                    (*p_addr)[i][j].skill[k] = &attack_skill;
                else
                    (*p_addr)[i][j].skill[k] = &heal_skill;
            }
        }
    }
}
...
{
    int random;
    if ((*enemy_addr).level < 5)
        random = rand() % 1;
    else if ((*enemy_addr).level < 10)
        random = rand() % 2;
    else
        random = rand() % 3;
    (*enemy_addr).skill[random]((*enemy_addr).skill_info[random].skill_name, random + 1, enemy_addr, my_addr);
}
...
int change_pokemon(Player *p1, int total_poke)
{
    ...
    while (1)
    {
        ...
        scanf("%d", &num);
        if (p1->battle_pokemon[num - 1].hp <= 0)
            printf("이 포켓몬은 기절했습니다!\n다른 포켓몬을 선택해주세요.\n");
        else
        {
            system("clear");
